...
from Sobel_glcic.Sobelmodels_test import CompletionNetwork, ContextDiscriminator
...
DEFAULT_RESULT_DIR = "D:/PycharmProjects/GLCIC-PyTorch-master/GLCIC-PyTorch-master/results/new_sobel"  # 默认结果保存路径
...
def main(args):
    ...
    try:
        # 1. 处理MPV格式（适配JSON序列化）
        mpv_json = [float(mpv[i]) for i in range(3)]
        # 2. 整合所有训练参数（命令行参数+MPV）
        args_dict = vars(args)
        args_dict['mpv'] = mpv_json  # 将MPV添加到参数字典
        # 3. 设置配置文件路径（保存到result_dir内部，方便查找）
        config_path = os.path.join(args.result_dir, 'config.json')
        # 4. 写入配置文件（添加indent=4，让JSON格式更易读）
        with open(config_path, mode='w', encoding='utf-8') as f:
            json.dump(args_dict, f, indent=4)
        # 5. 输出成功日志（确认保存路径）
        print(f"配置文件已成功保存到: {config_path}")
    except Exception as e:
        # 6. 捕获所有异常，避免程序崩溃并输出错误信息
        print(f"⚠️  保存配置文件时出错: {str(e)}")
        # 即使保存失败，也打印参数（避免参数丢失）
        print("当前训练参数如下（供记录）:")
    ...
    while pbar.n < args.steps_1:
        for x in train_loader:
            ...
            edge_map = gen_sobel_edge(x, ksize=3)  # 生成边缘图
            ...
            output_edge = gen_sobel_edge(output, ksize=3)
    ...
    while pbar.n < args.steps_2:
        for x in train_loader:
            ...
            mask = gen_input_mask(
                ...
                max_holes=args.max_holes).to(gpu)
            edge_map = gen_sobel_edge(x, ksize=3)  # 新增边缘图
...
# ================================================
    ...
    while pbar.n < args.steps_3:
        for x in train_loader:
            ...
            output_edge = gen_sobel_edge(output_cn, ksize=3)
