...
class CompletionNetwork(nn.Module):
    def __init__(self, input_channels=5):  # 修改输入通道为5 (RGB+Mask+Edge)
        super(CompletionNetwork, self).__init__()
        # 第一层适配新的输入通道
        # 原因：forward中x_main是前4通道（RGB+Mask），需与conv1输入通道匹配
    ...
    def forward(self, x):
        ...
        # ========== 修改标注2：补充通道分割说明（逻辑不变，仅加注释） ==========
        # 输入x为5通道：[B,5,H,W] = [RGB(3) + Mask(1) + Edge(1)]
        # 分割为：x_main（前4通道：RGB+Mask）、edge_map（第5通道：边缘图）
